...
TEST(MathLibInterpolationAlgorithms, PiecewiseLinearInterpolation)
{
    ...
    // Extrapolation
    ASSERT_NEAR(0, interpolation.getValue(-0.5), std::numeric_limits<double>::epsilon());
    // Extrapolation
    ASSERT_NEAR(1, interpolation.getValue(size-0.5), std::numeric_limits<double>::epsilon());
}
...
TEST(MathLibInterpolationAlgorithms, PiecewiseLinearInterpolationDerivative)
{
	...
	for (std::size_t k(0); k<size - 1; ++k) {
		...
		ASSERT_NEAR(1+2*k, interpolation.GetCurveDerivative(k + 0.5, true), std::numeric_limits<double>::epsilon());
	}

	// Extrapolation
	ASSERT_NEAR(1, interpolation.GetCurveDerivative(-1,true), std::numeric_limits<double>::epsilon());
	// Extrapolation
	ASSERT_NEAR(1997, interpolation.GetCurveDerivative(1001,true), std::numeric_limits<double>::epsilon());
}
