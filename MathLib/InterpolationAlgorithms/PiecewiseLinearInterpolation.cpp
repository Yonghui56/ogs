...
namespace MathLib
{
    ...
    {
		double var_slope, var_slope_invert, lim;
        ...
        if (interval_idx > 1 && interval_idx < interval_max - 2) {
			...
			var_slope =(1. - w) * s1 + w * s2;
        }
        else {
            if (std::fabs(_values_at_supp_pnts[interval_idx] - _values_at_supp_pnts[interval_idx + 1])>DBL_MIN)
				var_slope = (_supp_pnts[interval_idx] - _supp_pnts[interval_idx + 1]) / (_values_at_supp_pnts[interval_idx] - _values_at_supp_pnts[interval_idx + 1]);
            else
				var_slope = 1 / DBL_EPSILON;
        }
		lim = (-1.0) / DBL_EPSILON;
		//
		if (invert)
		{ // Return dSw/dPc
			var_slope_invert = 1.0 / var_slope;
			if (var_slope_invert < lim)
				var_slope_invert = lim;
			return var_slope_invert;
		}
		else
		{ // Return dPc/dSw
			if (var_slope < lim)
