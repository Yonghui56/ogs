 ...
 *
 */
#include "CreateTwoPhaseNCompCarbonationProcess.h"
#include <cassert>
...
#include "TwoPhaseNCompCarbonationProcess.h"
#include "TwoPhaseNCompCarbonationProcessData.h"
namespace ProcessLib
{
namespace TwoPhaseNCompCarbonation
{
std::unique_ptr<Process> createTwoPhaseNCompCarbonationProcess(
    MeshLib::Mesh& mesh,
        ...
        curves)
{
    ...
    config.checkConfigParameter("type", "TWOPHASE_NCOMP_CARBONATION");

    DBUG("Create TwoPhaseFlowProcess with PP model.");
    //! \ogs_file_param{prj__processes__process__TWOPHASE_NComp_CARBONATION__process_variables}
    auto const pv_config = config.getConfigSubtree("process_variables");

    auto process_variables = findProcessVariables(
        variables, pv_config,
        {//! \ogs_file_param_special{prj__processes__process__TWOPHASE_NComp_CARBONATION__process_variables__gas_pressure}
         "gas_pressure",
    ...
    std::vector<double> const b =
        //! \ogs_file_param{prj__processes__process__TWOPHASE_NComp_CARBONATION__specific_body_force}
    ...
    material = createTwoPhaseNCompCarbonationMaterialProperties(mat_config, material_ids);
    ...
    return std::unique_ptr<Process>{new TwoPhaseNCompCarbonationProcess{
