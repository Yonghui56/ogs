...
public:
				...
				element, is_axially_symmetric, _integration_method))
    {
		...
		_secondary_data._saturation.resize(ShapeFunction::NPOINTS);
		_secondary_data._liquid_pressure.resize(ShapeFunction::NPOINTS);
                  ...
                  std::vector<double>& local_b_data) override
    {
		...
		Eigen::MatrixXd mass_mat_coeff = Eigen::MatrixXd::Zero(NUM_NODAL_DOF, NUM_NODAL_DOF);
		...
		Eigen::MatrixXd localMass_tmp = Eigen::MatrixXd::Zero(ShapeFunction::NPOINTS, ShapeFunction::NPOINTS);
		...
		Eigen::VectorXd localSource_tmp = Eigen::VectorXd::Zero(ShapeFunction::NPOINTS); 

        auto local_M = MathLib::createZeroedMatrix<NodalMatrixType>(
            local_M_data, local_matrix_size, local_matrix_size);//mass matrix
        auto local_K = MathLib::createZeroedMatrix<NodalMatrixType>(
            local_K_data, local_matrix_size, local_matrix_size);//laplace matrix
		...
		MathLib::PiecewiseLinearInterpolation const&  interQ_slow = *_process_data.curves.at("curveA");
		MathLib::PiecewiseLinearInterpolation const&  interQ_fast = *_process_data.curves.at("curveB");
        ...
        for (unsigned ip = 0; ip < n_integration_points; ip++)
        {
            ...
            auto const rho_l_std = _process_data.water_density(t, pos)[0];//water density

			NumLib::shapeFunctionInterpolate(local_x, sm.N, PG_int_pt, X1_int_pt, X2_int_pt, X3_int_pt, PC_int_pt);
			...
			X_G_air_gp = get_X_G_air_gp(PG_int_pt, X1_int_pt, X2_int_pt, X3_int_pt, P_sat_gp);
			X_G_h2o_gp = get_X_G_h2o_gp(PG_int_pt, X1_int_pt, X2_int_pt, X3_int_pt, P_sat_gp);
			//double X_L_h2o_gp = X_G_h2o_gp *K_G_w;
			//double dXgairdPG_test = (_EOS->get_X_G_air_gp(PG_int_pt  + eps, X1_int_pt, X2_int_pt, X3_int_pt , P_sat_gp ) - X_G_air_gp ) / eps;
			double dLdPG = -X1_int_pt / Hen_L_h - X2_int_pt / Hen_L_c - X3_int_pt / Hen_L_co2;
			dXgairdPG = ((G / P_sat_gp - dLdPG)* (K_G_w - K_G_air) - (K_G_w*G - L)*(1 / P_sat_gp - 1 / Hen_L_air))
				/ (K_G_w - K_G_air) / (K_G_w - K_G_air);
			dXgh2odPG = -((G / Hen_L_air - dLdPG)*(K_G_w - K_G_air) + (L - K_G_air*G)*(1 / P_sat_gp - 1 / Hen_L_air))
				/ (K_G_w - K_G_air) / (K_G_w - K_G_air);
			...
			_secondary_data._saturation[ip] = S_G_gp;//store the secondary variable
			PL_gp = PG_int_pt - PC_int_pt;
			_secondary_data._liquid_pressure[ip] = PL_gp;//store the secondary variable
			dSgdPC = get_diriv_PC(PC_int_pt,4);//dSgdPC

			rho_mol_G_gp = PG_int_pt / R / T_0;
			rho_mol_L_gp = rho_l_std / M_L;
			rho_mass_G_gp = rho_mol_G_gp*(X1_int_pt*M_H + X2_int_pt *M_C + X3_int_pt*M_CO2 +
				X_G_air_gp*M_AIR + X_G_h2o_gp*M_L);
			rho_mass_L_gp = rho_l_std;

			
			Q_organic_slow_co2_ini = interQ_slow.getValue(t);//read from curves
			Q_organic_fast_co2_ini = interQ_fast.getValue(t);//read from curves

			/*calculate each entry of mass matrix*/
			mass_mat_coeff(0,0) = poro*(S_G_gp * X1_int_pt / R / T_0 + (1 - S_G_gp)*rho_mol_L_gp * X1_int_pt / Hen_L_h);//dPG
			mass_mat_coeff(0, 1) = poro*(rho_mol_G_gp * S_G_gp + (1 - S_G_gp)*rho_mol_L_gp * PG_int_pt / Hen_L_h);//dX1 h2
			mass_mat_coeff(0, 2) = 0.0;//Dx2 ch4
			mass_mat_coeff(0, 3) = 0.0;//dx3 CO2
			mass_mat_coeff(0, 4) = poro*(rho_mol_G_gp * X1_int_pt*dSgdPC - rho_mol_L_gp *X_L_h_gp *dSgdPC);//dPC
																														  //CH4
			mass_mat_coeff(1, 0) = poro*(S_G_gp*X2_int_pt / R / T_0 + (1 - S_G_gp)*rho_mol_L_gp *X2_int_pt / Hen_L_c);//dPG
			mass_mat_coeff(1, 1) = 0.0;//dX1 h2
			mass_mat_coeff(1, 2) = poro*(rho_mol_G_gp *S_G_gp + (1 - S_G_gp)*rho_mol_L_gp *PG_int_pt  / Hen_L_c);//Dx2 ch4
			mass_mat_coeff(1, 3) = 0.0;//dx3 CO2
			mass_mat_coeff(1, 4) = poro*(rho_mol_G_gp *X2_int_pt*dSgdPC - rho_mol_L_gp *X_L_c_gp *dSgdPC);//dPC
																														  //co2
			mass_mat_coeff(2, 0) = poro*(S_G_gp*X3_int_pt  / R / T_0 + (1 - S_G_gp)*rho_mol_L_gp *X3_int_pt  / Hen_L_co2);//dPG
			mass_mat_coeff(2, 1) = 0.0;//dX1 h2
			mass_mat_coeff(2, 2) = 0.0;//Dx2 ch4
			mass_mat_coeff(2, 3) = poro*(rho_mol_G_gp *S_G_gp + (1 - S_G_gp)*rho_mol_L_gp *PG_int_pt  / Hen_L_co2);//dx3 CO2
			mass_mat_coeff(2, 4) = poro*(rho_mol_G_gp *X3_int_pt *dSgdPC - rho_mol_L_gp *X_L_co2_gp *dSgdPC);//dPC
																															//air
			mass_mat_coeff(3, 0) = poro*(S_G_gp*X_G_air_gp  / R / T_0 + S_G_gp*rho_mol_G_gp *dXgairdPG
			...
			mass_mat_coeff(3, 1) = poro*(S_G_gp*rho_mol_G_gp *dxgairdX1
				+ (1 - S_G_gp)*rho_mol_L_gp *K_G_air*dxgairdX1);//dX1 h2

			mass_mat_coeff(3, 2) = poro*(S_G_gp*rho_mol_G_gp *dxgairdX2
				+ (1 - S_G_gp)*rho_mol_L_gp *K_G_air*dxgairdX2);//dX2 CH4

			mass_mat_coeff(3, 3) = poro*(S_G_gp*rho_mol_G_gp *dxgairdX3
				+ (1 - S_G_gp)*rho_mol_L_gp *K_G_air*dxgairdX3);//dX3 co2

			mass_mat_coeff(3, 4) = poro*(rho_mol_G_gp *X_G_air_gp *dSgdPC - rho_mol_L_gp *K_G_air*X_G_air_gp *dSgdPC);//dPC
																																		 //h20
			mass_mat_coeff(4, 0) = poro*(S_G_gp*X_G_h2o_gp  / R / T_0 + S_G_gp*rho_mol_G_gp *dXgh2odPG
			...
			mass_mat_coeff(4, 1) = poro*(S_G_gp*rho_mol_G_gp *dxgh2odX1
				+ (1 - S_G_gp)*rho_mol_L_gp *K_G_w*dxgh2odX1);//dX1 h2

			mass_mat_coeff(4, 2) = poro*(S_G_gp*rho_mol_G_gp *dxgh2odX2
				+ (1 - S_G_gp)*rho_mol_L_gp *K_G_w*dxgh2odX2);//dX2 CH4

			mass_mat_coeff(4, 3) = poro*(S_G_gp*rho_mol_G_gp *dxgh2odX3
				+ (1 - S_G_gp)*rho_mol_L_gp *K_G_w*dxgh2odX3);//dX3 co2

			mass_mat_coeff(4, 4) = poro*(rho_mol_G_gp *X_G_h2o_gp *dSgdPC - rho_mol_L_gp *K_G_w*X_G_h2o_gp *dSgdPC);//dPC
            //-------------debugging------------------------
			...
			Kr_L_gp = getKr_L_bySg(S_G_gp,4); // change the Relative permeability calculation into porous media file			
			...
			//h2
			K_mat_coeff(0, 0) = lambda_G*rho_mol_G_gp*X1_int_pt + lambda_L*rho_mol_L_gp*X_L_h_gp
				+ poro*D_L*(1 - S_G_gp)*rho_mol_L_gp*X1_int_pt / Hen_L_h;
			K_mat_coeff(0, 1) = poro*D_G*S_G_gp*rho_mol_G_gp  + poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *PG_int_pt / Hen_L_h;
			K_mat_coeff(0, 2) = 0.0;
			K_mat_coeff(0, 3) = 0.0;
			K_mat_coeff(0, 4) = -lambda_L*rho_mol_L_gp *X_L_h_gp ;
			//ch4
			K_mat_coeff(1, 0) = lambda_G*rho_mol_G_gp *X2_int_pt + lambda_L*rho_mol_L_gp *X_L_c_gp
				+ poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *X2_int_pt / Hen_L_c;
			K_mat_coeff(1, 1) = 0.0;
			K_mat_coeff(1, 2) = poro*D_G*S_G_gp*rho_mol_G_gp  + poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *PG_int_pt / Hen_L_c;
			K_mat_coeff(1, 3) = 0.0;
			K_mat_coeff(1, 4) = -lambda_L*rho_mol_L_gp *X_L_c_gp ;
			//co2
			K_mat_coeff(2, 0) = lambda_G*rho_mol_G_gp *X3_int_pt + lambda_L*rho_mol_L_gp *X_L_co2_gp
				+ poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *X3_int_pt / Hen_L_co2;
			K_mat_coeff(2, 1) = 0.0;
			K_mat_coeff(2, 2) = 0.0;
			K_mat_coeff(2, 3) = poro*D_G*S_G_gp*rho_mol_G_gp  + poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *PG_int_pt / Hen_L_co2;
			K_mat_coeff(2, 4) = -lambda_L*rho_mol_L_gp *X_L_co2_gp ;
			//air
			K_mat_coeff(3, 0) = lambda_G*rho_mol_G_gp *X_G_air_gp  + lambda_L*rho_mol_L_gp *K_G_air*X_G_air_gp
				+ poro*D_G*S_G_gp*rho_mol_G_gp *dXgairdPG
				+ poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *dXgairdPG*K_G_air;
			K_mat_coeff(3, 1) = poro*D_G*S_G_gp*rho_mol_G_gp *((-K_G_w + PG_int_pt / Hen_L_h) / (K_G_w - K_G_air))
				+ poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *dxgairdX1*K_G_air;
			K_mat_coeff(3, 2) = poro*D_G*S_G_gp*rho_mol_G_gp *((-K_G_w + PG_int_pt / Hen_L_c) / (K_G_w - K_G_air))
				+ poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *dxgairdX2*K_G_air;
			K_mat_coeff(3, 3) = poro*D_G*S_G_gp*rho_mol_G_gp *((-K_G_w + PG_int_pt / Hen_L_co2) / (K_G_w - K_G_air))
				+ poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *dxgairdX3*K_G_air;
			K_mat_coeff(3, 4) = -lambda_L*rho_mol_L_gp *K_G_air*X_G_air_gp ;
			//h2o
			K_mat_coeff(4, 0) = lambda_G*rho_mol_G_gp *X_G_h2o_gp  + lambda_L*rho_mol_L_gp *K_G_w*X_G_h2o_gp
				- poro*D_G*S_G_gp*rho_mol_G_gp *dXgairdPG
				- poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *(X1_int_pt / Hen_L_h + X2_int_pt / Hen_L_c + X3_int_pt / Hen_L_co2)
				- poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *dXgairdPG*K_G_air;
			K_mat_coeff(4, 1) = -poro*D_G*S_G_gp*rho_mol_G_gp
				- poro*D_G*S_G_gp*rho_mol_G_gp *dxgairdX1
				- poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *PG_int_pt / Hen_L_h
				- poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *dxgairdX1*K_G_air;
			K_mat_coeff(4, 2) = -poro*D_G*S_G_gp*rho_mol_G_gp
				- poro*D_G*S_G_gp*rho_mol_G_gp *dxgairdX2
				- poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *PG_int_pt / Hen_L_c
				- poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *dxgairdX2*K_G_air;
			K_mat_coeff(4, 3) = -poro*D_G*S_G_gp*rho_mol_G_gp
				- poro*D_G*S_G_gp*rho_mol_G_gp *dxgairdX3
				- poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *PG_int_pt / Hen_L_co2
				- poro*D_L*(1 - S_G_gp)*rho_mol_L_gp *dxgairdX3*K_G_air;
			...
			H_vec_coeff(0) = -lambda_G*rho_mol_G_gp*X1_int_pt*rho_mass_G_gp
				- lambda_L*rho_mol_L_gp*X1_int_pt*PG_int_pt*rho_mass_L_gp / Hen_L_h;
			H_vec_coeff(1) = -lambda_G*rho_mol_G_gp*X2_int_pt *rho_mass_G_gp
				- lambda_L*rho_mol_L_gp *X2_int_pt *PG_int_pt *rho_mass_L_gp  / Hen_L_c;
			H_vec_coeff(2) = -lambda_G*rho_mol_G_gp *X3_int_pt *rho_mass_G_gp
				- lambda_L*rho_mol_L_gp *X3_int_pt *PG_int_pt *rho_mass_L_gp  / Hen_L_co2;
			H_vec_coeff(3) = -lambda_G*rho_mol_G_gp *X_G_air_gp *rho_mass_G_gp 
				- lambda_L*rho_mol_L_gp *X_G_air_gp *PG_int_pt *rho_mass_L_gp  / Hen_L_air;
			H_vec_coeff(4) = -lambda_G*rho_mol_G_gp *X_G_h2o_gp *rho_mass_G_gp 
				- lambda_L*rho_mol_L_gp *X_G_h2o_gp *PG_int_pt *rho_mass_L_gp  / P_sat_gp ;
			...
			if (_process_data.has_gravity) {
				for (int idx = 0; idx < NUM_NODAL_DOF; idx++) {
					...
					local_b.block(n_nodes*idx, 0, n_nodes, 1).noalias() += localGravity_tmp;
				}
			...
			if (mat_id == 1) {
				...
				F_vec_coeff(1) += Q_organic_slow_co2 * 31 / 19;//CH4 organic degradation slow
				F_vec_coeff(2) += Q_organic_slow_co2;//co2 organic degradation slow
				F_vec_coeff(4) -= 2 * Q_organic_slow_co2; //h2o consume slow
				F_vec_coeff(1) += Q_organic_fast_co2;//co2 organic degradation fast
				F_vec_coeff(2) += Q_organic_fast_co2;//ch4 organic degradation fast
				...
				if (X3_int_pt > 0.01) {
					...
					F_vec_coeff(2) -= Q_organic_slow_co2; //carbonation consume co2
			...
			for (int idx = 0; idx < 5; idx++) {
				...
				localSource_tmp = sm.N.transpose()*F_vec_coeff(idx)*sm.detJ * wp.getWeight();
				local_b.block(n_nodes*idx, 0, n_nodes, 1).noalias() += localSource_tmp;
...
private:
	...
	SecondaryData<typename ShapeMatrices::ShapeType> _secondary_data;//secondary variables
